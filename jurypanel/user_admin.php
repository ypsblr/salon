...
if (isset($_SESSION['jury_yearmonth'])) {
	$sql = "SELECT * FROM contest WHERE yearmonth = '" . $_SESSION['jury_yearmonth'] . "' AND results_ready = 0 ";
...
// If there is no luck with jury_yearmonth - find the latest contest that is open
if (! isset($jury_yearmonth)) {
	// Get list of contests for dropdown box
	$sql = "SELECT * FROM contest WHERE results_ready = 0 ORDER BY yearmonth DESC";
	$query = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
	if (mysqli_num_rows($query) > 0) {
...
<head>
    <!-- Page title -->
	<title>Youth Photographic Society | User Administration Panel</title>
	...
	<div id="wrapper">
		<div class="content animate-panel">
			<div class="row">
							...
							<form action="op/add_user.php" id="loginForm" method="post" enctype="multipart/form-data">
								...
								?>
									...
									<div class="form-group col-lg-10">
										...
										<input type="file" class="form-control" name="picture" accept=".jpg,.jpeg" onchange="loadPhoto(this);" >
									</div>
								<?php
									// if(isset($_REQUEST['edit_id'])) {
								?>
						...
						<div class="panel-body">
							...
							?>
								<tr>
									...
									<td>
										<a href="user_admin.php?delete_id=<?php echo $tr_users['user_id'];?>" method="post"
				...
				<?php
					if (isset($jury_yearmonth) && $contest_list[$jury_yearmonth]['results_ready'] == 0) {
				?>
				<!-- JURY ASSIGH -->
				<div class="col-md-6 col-lg-6">
					<div class="hpanel">
						<div class="panel-body">
							...
							<form action="op/jury_assign.php" id="assignment_form" method="post" onsubmit="return validate_form()" enctype="multipart/form-data">
								<div class="form-group">
									...
									<select class="form-control" name="jury_yearmonth" value="<?php echo isset($jury_yearmonth) ? $jury_yearmonth : '';?>" >
									<?php
							...
							<?php
								...
								$sql .= " WHERE assignment.yearmonth = '" . (isset($jury_yearmonth) ? $jury_yearmonth : '') . "' ";
								$sql .= "   AND section.yearmonth = '" . (isset($jury_yearmonth) ? $jury_yearmonth : '') . "' ";
							...
							<?php
								$sql = "SELECT section FROM section WHERE yearmonth = '" . (isset($jury_yearmonth) ? $jury_yearmonth : '') . "' ORDER BY section";
								$query = mysqli_query($DBCON, $sql)or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
								while ($row = mysqli_fetch_array($query)) {
							?>
												<option value="<?php echo $row['section'];?>"><?php echo $row['section'];?></option>
							...
							?>
										...
										<div class="form-group col-lg-4 col-md-4 col-sm-4 col-xs-4" >
											<select class="form-control" data-visible="user_id" name="user_id[<?php echo $idx;?>]" id="user_id[<?php echo $idx;?>]" required>

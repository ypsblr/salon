...
function prepare_pic_entry($stat_yearmonth, $award_group, $stat_category, $stat_segment, $stat_segment_sequence) {
	...
	$sql = <<<SQL
		UPDATE pic
		   SET pic.total_rating = (SELECT SUM(rating) FROM rating WHERE rating.yearmonth = pic.yearmonth AND rating.profile_id = pic.profile_id AND rating.pic_id = pic.pic_id)
...
// ============================
//
$sql = "SELECT * FROM stats_def WHERE yearmonth = '$stat_yearmonth' AND stat_refresh != 0";
$query = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
while ($row = mysqli_fetch_array($query)) {
	if ( $row['stat_routine'] ($row['yearmonth'], $row['award_group'], $row['stat_category'], $row['stat_segment'], $row['stat_segment_sequence']) ) {
		$sql  = "UPDATE stats_def ";
		$sql .="    SET stat_refresh = 0 ";
		$sql .= " WHERE yearmonth = '" . $row['yearmonth'] . "' ";
		$sql .= "   AND award_group = '" . $row['award_group'] . "' ";
		$sql .= "   AND stat_category = '" . $row['stat_category'] . "' ";
		$sql .= "   AND stat_segment = '" . $row['stat_segment'] . "' ";
		mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
	}
}

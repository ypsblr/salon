...
if($resultsReady) {
	// Salon Participation Statistics
	$sql  = "SELECT count(*) AS num_entries FROM entry ";
	$sql .= " WHERE yearmonth = '$contest_yearmonth' ";
	$sql .= "   AND profile_id IN (SELECT profile_id FROM pic WHERE yearmonth = '$contest_yearmonth')";
	...
	$sql  = "SELECT count(*) AS num_pictures FROM pic ";
	$sql .= " WHERE yearmonth = '$contest_yearmonth' ";
	$sql .= "   AND profile_id IN (SELECT profile_id FROM entry WHERE yearmonth = '$contest_yearmonth')";
	$qtmp = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
	$rtmp = mysqli_fetch_array($qtmp);
	$contestNumPictures = $rtmp['num_pictures'];
	
	$sql  = "SELECT DISTINCT country FROM entry ";
	$sql .= " WHERE yearmonth = '$yearmonth' ";
	$sql .= "   AND profile_id IN (SELECT DISTINCT profile_id FROM pic WHERE yearmonth = '$contest_yearmonth')";
	$qtmp = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);

...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
// Following line changed to escape hyphen (-) characters within square brackets to avoid the same being treated as a range of characters
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-:]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        $pattern = "/([\w\-:\*]*)(?:\#([\w\-]+)|\.([\w\-]+))?(?:\[@?(!?[\w\-:]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    */
    ...
    function get_display_size()
    {
        ...
        if (isset($this->attr['style']))
        {
            ...
            $attributes = array();
            // Following line changed by escaping hyphen (-) as hyphen is treated as a special character (range) in PHP 7.3 onwards 
...
class simple_html_dom
{
    ...
    protected function read_tag()
    {
        ...
        // Following line changed as hyphen (-) is treated as a special character specifying range from PHP 7.3
        // if (!preg_match("/^[\w-:]+$/", $tag)) {
        if (!preg_match("/^[\w\-:]+$/", $tag)) {

...
class ImageMerge {
	...
	protected $zip_name = "title_cards.zip";
	...
	function __construct($dbcon, $param) {
		...
		$this->zip_folder = $_SERVER['DOCUMENT_ROOT'] . "/generated/" . $this->yearmonth . "/title_cards";
		// $this->target_folder = $_SERVER['DOCUMENT_ROOT'] . "/generated/" . $this->yearmonth . "/title_cards/" . $this->section;

		// Load Config JSON File
		if (file_exists($this->salon_folder . "/blob/title_card.json")) {
			$this->conf = json_decode(file_get_contents($this->salon_folder . "/blob/title_card.json"), true);
			if (json_last_error() != JSON_ERROR_NONE)
				$this->return_error("Title Card Configuration Corrupt. Use Salon Manage to create a Title Card Configuration.", __FILE__, __LINE__);
			if ($this->yearmonth != $this->conf['yearmonth'])
				$this->return_error("Title Card Configuration File does not relate to this contest.", __FILE__, __LINE__);
		}
		else
			$this->return_error("Title Card Configuration File ($this->salon_folder/blob/title_card.json) not found. Use Salon Manage to create a Title Card Configuration.", __FILE__, __LINE__);

		if (! file_exists($this->salon_folder . "/img/" . $this->conf['design']))
			$this->return_error("Title Card Background Design Image File not found.", __FILE__, __LINE__);
		...
		if ($this->section != "")
			$this->sql .= "   AND award.section = '" . $this->section . "' ";					// Remove this test code
	...
	protected function us_space($str) {
	...
	function getMergeData() {
		...
		if ($row = mysqli_fetch_array($this->query)) {
			...
			$this->save_file_name = sprintf("%s-%s-%s-%04d-%02d%s", $this->conf['file_name_stub'], $row['stub'], $this->us_space($row['award_name']), $row['profile_id'], $row['pic_id'], self::SAVE_FILE_TYPE);
			$this->zip_file_name = $row['section'] . "/" . $this->save_file_name;
			// $this->target_folder = $_SERVER['DOCUMENT_ROOT'] . "/generated/" . $this->yearmonth . "/titlecards/" . $row['section'];
			// $this->save_file_name = sprintf("%s-%s-%s-%04d-%02d%s", $this->conf['file_name_stub'], $row['stub'], $this->us_space($row['award_name']), $row['profile_id'], $row['pic_id'], self::SAVE_FILE_TYPE);
			foreach ($this->conf['picture'] as $index => $config) {
				...
				elseif ($config['field'] == 'avatar') {
					if ($row['avatar'] == "" || $row['avatar'] == "user.jpg")
						$this->conf['picture'][$index]["value"] = "";
					else {
						$avatar = $_SERVER['DOCUMENT_ROOT'] . "/res/avatar/" . $row['avatar'];
						$this->conf['picture'][$index]["value"] = $avatar;
					}
				}
				...
				elseif ($this->conf['is_international'] == '1' && $config['field'] == "flag") {
					...
					if (file_exists($flag_img)) {
						$this->conf['picture'][$index]["value"] = $flag_img;
					}
					else
						$this->conf['picture'][$index]["value"] = "";
				}
				elseif (isset($row[$config['field']]))
					$this->conf['picture'][$index]["value"] = $row[$config['field']];
			}
			// $this->debug_dump("picture", $this->conf['picture'], __FILE__, __LINE__);

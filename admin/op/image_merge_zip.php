...
if ( (! empty($_REQUEST['yearmonth'])) && (! empty($_REQUEST['merge'])) ) {
	...
	// Create ZIP File
	$zip_folder = $doc->getZipFolder();
	$zipfile = $zip_folder . "/" . $doc->getZipName();
	$zip = New ZipArchive;
	...
	while ($fields = $doc->getMergeData()) {
		set_time_limit(60);				// Restart 60 second timer for each page
		...
		if ( ! $zip->addFile($savefilename, $doc->getZipFileName()))
			die_with_error("Error adding " . $doc->getZipFileName() . " to ZIP");

		// $image->save($save_folder, $doc->getSaveFileName(), true, null, $imageQuality = 85, $interlace = false);
		++ $count;
	}
	set_time_limit(300);				// Restart 300 seconds to finish
	// Download ZIP File
	$zip->close();

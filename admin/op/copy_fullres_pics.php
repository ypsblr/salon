...
if( isset($_SESSION['admin_yearmonth']) && isset($_SESSION['admin_id']) && isset($_REQUEST['section']) && isset($_REQUEST['start']) && isset($_REQUEST['size']) ) {
	...
	define("PIC_FOLDER", SALON_HOME . "/salons/$yearmonth/upload/$section/full");
	define("ZIP_FOLDER", SALON_HOME . "/generated/$yearmonth/fullres_pics/$section");

	if (! is_dir(ZIP_FOLDER)) {
		make_path(ZIP_FOLDER);
	}

	$zipfile = ZIP_FOLDER . "/" . sprintf("%04d_fullres_pics.zip", $start + 1);
	$zip = New ZipArchive;
	...
	$sql  = "SELECT pic.profile_id, profile.profile_name, pic.pic_id, pic.title, pic.full_picfile, award.award_id, award.award_name  ";
	...
	while ($rpic = mysqli_fetch_array($qpic)) {
		$upload_pic = PIC_FOLDER . "/" . $rpic['full_picfile'];
		$zip_pic = "full/" . $rpic['full_picfile'];

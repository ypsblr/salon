...
require __DIR__ . "/../inc/CryptoJsAes.php";
...
function send_salon_email($email_id, $subject, $email_code, $text, $replace_text, $call_backs = false) {
	...
	if ($row = mysqli_fetch_array($query)) {
        if ( ! empty($contest_yearmonth) && file_exists("../salons/$contest_yearmonth/blob/" . $row['email_body_blob']))
            $email_format = file_get_contents("../salons/$contest_yearmonth/blob/" . $row['email_body_blob']);
        else
		...
		if (preg_match("/localhost/i", $_SERVER['SERVER_NAME'])) {
            ...
            try {
                ...
                $mail->Host       = 'localhost';                        // For GoDaddy
                $mail->SMTPAuth   = false;                              // For GoDaddy
                $mail->SMTPAutoTLS   = false;                           // For GoDaddy
                $mail->Port       = 25;                                 // For GoDaddy
...
function handle_error($errmsg, $phpfile = "<Not provided>", $phpline = "<Not provided>") {
...
function yps_login($login_id, $password, $email = "") {
	...
	if ($result && $result['status'] == "OK") {
		...
		$err_msg = "Validation Failed !!! " . ($result ? $result['errmsg'] . "!" : "");
	}

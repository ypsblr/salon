...
function sync_yps_login_id($is_yps_member, $yps_user, $profile) {
	global $DBCON;
...
//
// echo '<script>console.log("Handling signup request")</script>';
if (isset($param['sign_up'])) {
    ...
    echo '<script>console.log("Successfully got profile")</script>';
	//
	...
	if ( $profile == false )	{
		...
		debug_to_console($param['login_id'], $err_msg, $is_yps_member);
		if ($is_yps_member) {
		    echo '<script>console.log("Is YPS member !!")</script>';
	...
	echo '<script>console.log("Dying without doing anything")</script>';
	die();
}
...
if(isset($param['login_reset_password'])) {
	...
	$text = array('{{NAME}}', '{{EMAIL}}', '{{PASSWORD}}', '{{URL}}');
	send_salon_email($row['email'], "Your request completed", 'password_reset', $text, $replace_text);
	handle_error("Your YPS membership is no more valid. Hence a temporary password has been generated and sent to your email. Please wait for the email and login using this password.", __FILE__, __LINE__);
...
if(isset($param['login_check']) && (! empty($param['login_id'])) && (! empty($param['login_password'])) ) {
    ...
    debug_to_console(5.1);
	$profile = get_user_profile($login_email, $login_id);
    debug_to_console(5.2);

	if ($profile == false) {
	    debug_to_console(5.3);
		// There is no profile yet`
		// Check if the person is a YPS Member
		list($err_msg, $is_yps_member, $yps_user) = yps_getuserbyemail($login_id);
		if ($is_yps_member) {
		    debug_to_console(5.4);
			$yps_login_id = $yps_user['yps_login_id'];
			// Use the email ID on YPS member database to find salon user
			$profile = get_user_profile($yps_user['email'], strtoupper($yps_user['email']));
			if ($profile == false) {
			    debug_to_console(5.5);
				// Still no profile found. exit
				handle_error("You are yet to Sign Up. Please Sign Up now using your Email or YPS Member ID.", __FILE__, __LINE__);
			}
			else {
			    debug_to_console(5.6);
				// Found salon profile. Fill yps_login from YPS member database
				$profile_id = $profile['profile_id'];
				sync_yps_login_id($is_yps_member, $yps_user, $profile);
				// Update login_id in salon profile
				// if ($profile['yps_login_id'] != $yps_user['yps_login_id']) {
				// 	$sql = "UPDATE profile SET yps_login_id = '$yps_login_id', password = '' WHERE profile_id = '$profile_id' ";
				// 	mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
				// }
	    ...
	    debug_to_console(5.7);
		// Profile exists
		$profile_id = $profile['profile_id'];
		list($err_msg, $is_yps_member, $yps_user) = yps_getuserbyemail($profile['email']);
		sync_yps_login_id($is_yps_member, $yps_user, $profile);
	}

	if ($profile_id == 0) {
	    debug_to_console(5.8);
		handle_error("You are yet to Sign Up. Please Sign Up now using your Email or YPS Member ID.", __FILE__, __LINE__);
	}

    debug_to_console(5.9);
	// Matching profile found
	list($err_msg, $is_yps_member, $yps_user) = yps_getuserbyemail($profile['email']);
	debug_to_console(5.10);
	sync_yps_login_id($is_yps_member, $yps_user, $profile);
    debug_to_console(5.11);
	...
	debug_to_console(5.12);
	$sql = "SELECT * FROM profile WHERE profile_id = '$profile_id' ";
	$query = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
	$profile = mysqli_fetch_array($query);
    debug_to_console(5.13);

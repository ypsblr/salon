...
if( isset($_SESSION['admin_id']) && isset($_REQUEST['save-json']) && isset($_REQUEST['yearmonth']) ) {
	...
	$json_file = $target_folder . "/" . "slideshow.json";

	// Assemble Section Opening, Section Closing and Picture Field Definitions
	if (isset($_REQUEST['opening_data']) && sizeof($_REQUEST['opening_data']) > 0) {
		$section_opening = [];
		foreach($_REQUEST['opening_data'] as $data_json) {
			$section_opening[] = json_decode($data_json, true);
		}
	}
	else {
		return_error("Section Opening Field Definitions Empty", __FILE__, __LINE__, true);
	}
	if (isset($_REQUEST['closing_data']) && sizeof($_REQUEST['closing_data']) > 0) {
		$section_closing = [];
		foreach($_REQUEST['closing_data'] as $data_json) {
			$section_closing[] = json_decode($data_json, true);
		}
	}
	else {
		return_error("Section Closing Field Definitions Empty", __FILE__, __LINE__, true);
	}
	...
	$slideshow = array(
		...
		"slideshow_design" => $_REQUEST['slideshow_design'],
	...
	file_put_contents($json_file, json_encode($slideshow));

	$resArray = [];
	$resArray['success'] = TRUE;
	$resArray['msg'] = "";
	$resArray['slideshow'] = $slideshow;

...
function get_ribbon_holder_config($yearmonth) {

	if (file_exists(__DIR__ . "/../salons/$yearmonth/blob/ribbon_holder.json")) {
		// Load Partner Data
		$ribbon_holder_config = json_decode(file_get_contents(__DIR__ . "/../salons/$yearmonth/blob/ribbon_holder.json"), true);
		if (json_last_error() != JSON_ERROR_NONE) {
			$_SESSION['err_msg'] = "Ribbon Holder Definition garbled";
			return false;
		}

		return $ribbon_holder_config;
	}
	else {
		return false;
	}
...
$pic_image_fields = ["Picture" => "picfile"];
$pic_text_fields = ["Award Name" => "award_name", "Author" => "profile_name", "Honors" => "honors", "Pic Title" => "title", "Section" => "section" ];
...
if ( isset($_SESSION['admin_id']) ) {
	...
	// Clone previous conf
	if (isset($_REQUEST['yearmonth']) && isset($_REQUEST['copyfrom']) && file_exists("../salons/" . $_REQUEST['copyfrom'] . "/blob/ribbon_holder.json"))
		copy("../salons/" . $_REQUEST['copyfrom'] . "/blob/ribbon_holder.json", "../salons/" . $_REQUEST['yearmonth'] . "/blob/ribbon_holder.json");
	...
	if (isset($_REQUEST['yearmonth'])) {
		...
		if (! file_exists("../salons/$yearmonth/blob/ribbon_holder.json")) {
			...
			if (mysqli_num_rows($query) > 0) {
				...
				if (file_exists("../salons/$prev_yearmonth/blob/ribbon_holder.json")) {
					copy("../salons/$prev_yearmonth/blob/ribbon_holder.json", "../salons/$yearmonth/blob/ribbon_holder.json");
					$_SESSION['info_msg'] = "Copied Ribbon Holders Definition from $prev_contest_name.";
				}
			}
		}
		// Get Ribbon Holder Config
		if ($conf = get_ribbon_holder_config($yearmonth))
			$has_conf = true;

		// Check for backdrop design file
		$design_file = ($is_international ? "is" : "ais") . substr($yearmonth, 0, 4) . "_ribbon_holder.png";
		if (file_exists("../salons/$yearmonth/img/$design_file")) {
			list($rh_width, $rh_height) = getimagesize("../salons/$yearmonth/img/$design_file");
			$has_design = true;
		}
		// Previous year Conf
		$prev_conf = [];
		...
		while ($row = mysqli_fetch_array($query)) {
			if (file_exists("../salons/" . $row['yearmonth'] . "/blob/ribbon_holder.json"))
...
?>
	...
	<div id="wrapper">
			...
			<div class="hpanel">
				<div class="panel-body">
					...
					<h3 class="font-light m-b-xs">
						Configure Ribbon Holder
					</h3>
					<br>
					<form role="form" method="post" name="select-contest-form" action="ribbon_holder.php" enctype="multipart/form-data" >
		...
		<div class="content">
			<form role="form" method="post" id="edit-ribbon-holder-form" name="edit-ribbon-holder-form" action="#" enctype="multipart/form-data" >
				...
				<input type="hidden" name="width" id="width" value="<?= $rh_width;?>" >
				<input type="hidden" name="height" id="height" value="<?= $rh_height;?>" >
				...
				<div class="row form-group">
					<div class="col-sm-6">
						<label for="yearmonth">Select Salon from which to copy Ribbon Holder configuration</label>
						<div class="input-group">
				...
				<!-- Ribbon Holder Design -->
				<div class="row form-group">
					<div class="col-sm-9">
						<div class="row">
							<div class="col-sm-12"><b>Ribbon Holder Design *</b></div>
							<div class="col-sm-2">
					...
					<!-- Vertical Cutting Bleed -->
					<div class="col-sm-3">
						<label for="edit-bleed">Cutting Mark width (File W x H = <span id="edit-bleed-width"><?= $rh_width;?></span> x <span id="edit-bleed-height"><?= $rh_height;?></span>) *</label>
						<input type="number" name="bleed" id="edit-bleed"
								value="<?= $has_conf ? $conf['bleed'] : 0;?>" >
					</div>
				</div>
				<!-- Ribbon Holder Parameters -->
				<div class="row form-group">
					<!-- File Name Stub -->
					<div class="col-sm-3">
						<label for="edit-stub">Ribbon Holder File Stub *</label>
						<input type="text" name="stub" id="edit-stub"
								value="<?= $has_conf ? $conf['file_name_stub'] : ($is_international ? "IS" : "AIS") . substr($yearmonth, 0, 4) . "-RH";?>" >
					</div>
					<!-- Regular Font -->
					<div class="col-sm-3">
					...
					<div class="col-sm-12">
						...
						?>
								<tr id="picture-field-<?= $idx;?>-row">
									...
									<td>
										<button id="delete-picture-fields-<?= $idx;?>" class="btn btn-danger delete-field-btn"
												data-idx='<?= $idx;?>' >
...
<script>
	...
	$(document).ready(function() {
		...
		$("#select-prev-json-button").click(function(){
			...
			window.location.href="ribbon_holder.php?yearmonth=" + yearmonth + "&copyfrom=" + prev_contest;
		});
		// Edit button
...
<script>
	...
	$(document).ready(function(){
			...
			if (input.files && input.files.length > 0 && input.files[0] != "") {
				...
				reader.onload = function (e) {
					...
					$("#disp-design").one("load", function() {
						...
						$("#edit-bleed-width").html(this.naturalWidth);
						$("#height").val(this.naturalHeight);
						$("#edit-bleed-height").html(this.naturalHeight);
		...
		$("#upload-design").click(function(e){
			e.preventDefault();
			if ($("#edit-design").val() == "") {
				swal("Select a Ribbon Holder Design Image !", "Please select a Ribbon Holder design image to upload.", "warning");
			}
			else {
				let yearmonth = "<?= $yearmonth;?>";
		...
		$('#edit-field-form').validate({
			rules:{
				"edit-x" : {
					min : 0,
					max : $("#width").val() - (2 * $("#edit-bleed").val()),
					required : true,
				},
				"edit-y" : {
					min : 0,
					max : $("#height").val() - (2 * $("#edit-bleed").val()),
					required : true,
				},
				"edit-width" : {
					min : 0,
					max : ($("#width").val() - (2 * $("#edit-bleed").val()) - $("#edit-x").val()),
				},
				"edit-height" : {
					min : 0,
					max : ($("#height").val() - (2 * $("#edit-bleed").val()) - $("#edit-y").val()),
				},
				"edit-rotate" : {
					min : 0,
					max : 360,
	...
	// Save JSON
	$('#edit-ribbon-holder-form').validate({
		rules:{
		},
		messages:{
		},
		...
		submitHandler: function(form) {
			...
			if (! $("#disp-design").attr("src").endsWith("ribbon_holder.png")) {
				swal("No Ribbon Holder Design Image !", "Please select a Ribbon Holder design image and upload.", "warning");
				return false;
			}
			...
			$('#loader_img').show();
			$.ajax({
					url: "ajax/save_ribbon_holder_json.php",
					type: "POST",
					data: formData,
					cache: false,
					processData: false,
					...
					success: function(response) {
						...
						if(response.success){
							swal({
									title: "Ribbon Holder Definition Saved",
									text: "The Ribbon Holder definition has been saved. Run Ribbon Holder generation from Admin system.",
									icon: "success",
									confirmButtonClass: 'btn-success',
									confirmButtonText: 'Great'
							});
							...
							swal({
									title: "Save Failed",
									text: "Ribbon Holder Definition could not be saved: " + response.msg,
									icon: "warning",
									confirmButtonClass: 'btn-warning',
									confirmButtonText: 'OK'
							});

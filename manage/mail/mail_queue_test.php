...
$sql = "SELECT * FROM mail_queue WHERE status != 'COMPLETED' AND status != 'DELETED' AND is_test = 1 LIMIT 1 ";
...
foreach ($mail_to_list as $profile_id) {
	...
	if ($status == "OK") {
		...
		if ($email_message == "") {
	    	if (DEBUG_TRACE) echo "$ email_message is empty<br>";
			log_mail_error($queue_id, $profile_id, "failed", "Mail Merge failed");
		}
		else {
	    	if (DEBUG_TRACE) echo "$ email_message is not empty, trying to send the email<br>";
			if (send_mail($mail_to, $subject, $email_message, $cc_to)) {
    	    	if (DEBUG_TRACE) echo "$mail_to -- $subject -- $cc_to --<br>";
    	    	if (DEBUG_TRACE) echo "$email_message<br>";
    	    	if (DEBUG_TRACE) echo "email sent successfully<br>";
				++ $sent;
			}
			else {
    	    	if (DEBUG_TRACE) echo "Mail sending failed<br>";

...
class MailGenerator {
	...
	protected $guest_exists;
	protected $other_exists;
	protected $status = "OK";
	protected $errmsg = "";
	...
	function __construct($DBCON, $yearmonth, $is_salon_archived) {
		...
		$this->guest_exists = (! empty($dignitory_names[1]));
		$this->other_exists = (! empty($dignitory_names[2]));
	}
...
HTML;
	...
	function generatorPairs() {
		$pairs = array(
			...
			"exhibition-invite-exists" => ( ! empty($this->exhibition['invitation_img']) ),
			"exhibition-message-exists" => ( ! empty($this->exhibition['email_message_blob']) ),
			"exhibition-invite-img" => $this->exhibition['invitation_img'],
			"exhibition-message-blob" => $this->exhibition['email_message_blob'],
			"exhibition-chair-exists" => $this->chair_exists,
			"exhibition-guest-exists" => $this->guest_exists,
			"exhibition-other-exists" => $this->other_exists,
		);
		return $pairs;
	}
}

...
function get_config($yearmonth) {
	...
	if (file_exists(__DIR__ . "/../salons/$yearmonth/blob/title_card.json")) {
		// Load Partner Data
		$config = json_decode(file_get_contents(__DIR__ . "/../salons/$yearmonth/blob/title_card.json"), true);
		if (json_last_error() != JSON_ERROR_NONE) {
			$_SESSION['err_msg'] = "Title Card Definition garbled";
			return false;
		}
...
$pic_image_fields = ["Thumbnail" => "picfile", "Author Avatar" => "avatar", "Flag" => "flag"];
$pic_text_fields = ["Author Name" => "profile_name", "Honors" => "honors", "Author Country" => "country_name",
					"Award Name" => "award_name", "Picture Title" => "title", "Section" => "section"];
...
if ( isset($_SESSION['admin_id']) ) {
	...
	// Clone previous conf
	if (isset($_REQUEST['yearmonth']) && isset($_REQUEST['copyfrom']) && file_exists("../salons/" . $_REQUEST['copyfrom'] . "/blob/title_card.json"))
		copy("../salons/" . $_REQUEST['copyfrom'] . "/blob/title_card.json", "../salons/" . $_REQUEST['yearmonth'] . "/blob/title_card.json");
	...
	if (isset($_REQUEST['yearmonth'])) {
		...
		if (! file_exists("../salons/$yearmonth/blob/title_card.json")) {
			...
			if (mysqli_num_rows($query) > 0) {
				...
				if (file_exists("../salons/$prev_yearmonth/blob/title_card.json")) {
					copy("../salons/$prev_yearmonth/blob/title_card.json", "../salons/$yearmonth/blob/title_card.json");
					$_SESSION['info_msg'] = "Copied Title Card Definition from $prev_contest_name.";
				}
			}
		}
		// Get Config
		...
		$design_file = ($is_international ? "is" : "ais") . substr($yearmonth, 0, 4) . "_title_card.png";
		...
		while ($row = mysqli_fetch_array($query)) {
			if (file_exists("../salons/" . $row['yearmonth'] . "/blob/title_card.json"))
...
?>
	...
	<div id="wrapper">
			...
			<div class="hpanel">
				<div class="panel-body">
					...
					<h3 class="font-light m-b-xs">
						Configure Title Card
					</h3>
					<br>
					<form role="form" method="post" name="select-contest-form" action="title_card.php" enctype="multipart/form-data" >
		...
		<div class="content">
			<form role="form" method="post" id="edit-title-card-form" name="edit-title-card-form" action="#" enctype="multipart/form-data" >
				...
				<div class="row form-group">
					<div class="col-sm-6">
						<label for="yearmonth">Select Salon from which to copy Title Card configuration</label>
						<div class="input-group">
				...
				<div class="row form-group">
					<div class="col-sm-9">
						<div class="row">
							<div class="col-sm-12"><b>Title Card Design *</b></div>
							<div class="col-sm-2">
				...
				<div class="row form-group">
					<!-- File Name Stub -->
					<div class="col-sm-3">
						<label for="edit-stub">Title Card File Stub *</label>
						<input type="text" name="stub" id="edit-stub"
								value="<?= $has_conf ? $conf['file_name_stub'] : ($is_international ? "IS" : "AIS") . substr($yearmonth, 0, 4) . "-TC";?>" >
					</div>
					<!-- Regular Font -->
					<div class="col-sm-3">
...
<script>
	...
	$(document).ready(function() {
		...
		$("#select-prev-json-button").click(function(){
			...
			window.location.href = "title_card.php?yearmonth=" + yearmonth + "&copyfrom=" + prev_contest;
		});
		// Edit button
...
<script>
	...
	$(document).ready(function(){
		...
		$("#upload-design").click(function(e){
			e.preventDefault();
			if ($("#edit-design").val() == "") {
				swal("Select a Design Image !", "Please select a Title Card design image to upload.", "warning");
			}
			else {
				let yearmonth = "<?= $yearmonth;?>";
	...
	$('#edit-title-card-form').validate({
		...
		submitHandler: function(form) {
			...
			if (! $("#disp-design").attr("src").endsWith("title_card.png")) {
			...
			$('#loader_img').show();
			$.ajax({
					url: "ajax/save_title_card_json.php",
					type: "POST",
					data: formData,
					cache: false,
					processData: false,
					...
					success: function(response) {
						...
						if(response.success){
							swal({
									title: "Title Card Definition Saved",
									text: "The Title Card definition has been saved. Run Title Card generation from Admin system.",
									icon: "success",
									confirmButtonClass: 'btn-success',
									confirmButtonText: 'Great'
							});
							...
							swal({
									title: "Save Failed",
									text: "Title Card Definition could not be saved: " + response.msg,
									icon: "warning",
									confirmButtonClass: 'btn-warning',
									confirmButtonText: 'OK'
							});

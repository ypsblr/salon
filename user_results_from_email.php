<?php
// Open results directly from email
// PHP Settings
...
// $_REQUEST['code'] = encode_string_array("202108|1242");

if (empty($_REQUEST['code']))
	handle_error("Open from the results email", __FILE__, __LINE__);

list ($yearmonth, $profile_id) = explode("|", decode_string_array($_REQUEST['code']));

$sql = "SELECT * FROM contest WHERE yearmonth = '$yearmonth' ";
$query = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
$contest = mysqli_fetch_array($query);
$contest_archived = ($contest['archived'] == '1');
$resultsReady = ($contest['results_ready'] == '1');
$certificatesReady = ($contest['certificates_ready'] == '1');
$contest_yearmonth = $yearmonth;
$contestName = $contest['contest_name'];
$registrationLastDate = $contest["registration_last_date"];
$submissionTimezone = $contest['submission_timezone'];
$updateEndDate = $contest['update_end_date'];
$contestHasSponsorship = false;

// if ($yearmonth != $contest_yearmonth)
// 	handle_error("This link relates to an older contest", __FILE__, __LINE__);

if ( ! $resultsReady )
	handle_error("Results are yet to be announced", __FILE__, __LINE__);

$sql  = "SELECT * FROM pic ";
$sql .= " WHERE yearmonth = '$yearmonth' AND profile_id = '$profile_id' ";
$query = mysqli_query($DBCON, $sql) or sql_error($sql, mysqli_error($DBCON), __FILE__, __LINE__);
if (mysqli_num_rows($query) == 0)
	handle_error("Invalid participation link for this contest", __FILE__, __LINE__);

// Get User Profile
$sql  = "SELECT * FROM entry, profile LEFT JOIN club ON club.club_id = profile.club_id ";
...
// if (file_exists("salons/$contest_yearmonth/generate_share_image.php")) {
// if (file_exists("salons/$yearmonth/generate_share_image.php")) {
if (file_exists("salons/$yearmonth/blob/sharedef.json")) {
	try {
		$si_yearmonth = $yearmonth;
		$si_profile_id = $profile_id;
		include_once "inc/generate_share_image.php";
		// include_once "salons/$yearmonth/generate_share_image.php";
		// include_once "salons/$contest_yearmonth/generate_share_image.php";
	}
	catch (Exception $e) {
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9" id="myTab">
					<h2 class="text-color"><?= $tr_user['profile_name'];?></h2>
					<!-- SHARE IMAGE -->
					<?php
						...
						$poster_file = "salons/" . $yearmonth . "/upload/share/" . sprintf("yps-poster-%04d.jpg", $tr_user['profile_id']);
					...
					<div class="row">
						...
						<?php
							...
							if ($rprs['num_results'] > 0 && $certificatesReady && file_exists("salons/" . $yearmonth . "/certdef.php") ) {
								$upload_code = encode_string_array($yearmonth . "|PROFILE|" . $tr_user['profile_id'] . "|ALL");
						?>
					...
					<div class="row">
						...
						while ($rpic = mysqli_fetch_array($qpic)) {
							...
							if ($rejection_text != "")
								++ $total_rejections;
							$thumbnail = "/salons/$yearmonth/upload/$pic_section/tn/" . $rpic['picfile'];
						...
						<?php
							...
							if ($rawd = mysqli_fetch_array($qawd)) {
								...
								$upload_code = encode_string_array($yearmonth . "|" . $rawd['award_id'] . "|" . $tr_user['profile_id'] . "|" . $pic_id);
						?>
						...
						<?php
								...
								if ($rawd['has_certificate'] && $certificatesReady && file_exists("salons/" . $yearmonth . "/certdef.php") ) {
						?>
						...
						?>
										...
										<p class="text-center" style="color:<?=empty($rpic['full_picfile']) ? 'red' : 'green';?>;">
											<?= empty($rpic['full_picfile']) ? "Upload full resolution image for Catalog" : "You have already uploaded a file for printing";?>
										</p>
...
//						dontBreakRows : true,
		...
		$("#download_results").click(function() {
			...
			let pdf_def = {
					info : {
						title : "<?= $contest['contest_name'];?> - Salon Results of <?= ucwords(strtolower($tr_user['profile_name']));?>",
					...
					content : [
						...
						[ { text : "Summary of Results for <?= $contest['contest_name'];?>", style : "h3", margin : [0, 8, 0, 4] } ],
						[
							{
								layout : 'tight_cell',
								table : {
					...
					images : {
								...
								report_banner : '<?= http_method() . $_SERVER['SERVER_NAME'] . "/salons/$yearmonth/img/results_banner.jpg";?>',
					},
			};
			// Add Patronage house logos (same name as short_code)
			$(".patronage-data").each(function(index, recn){
				pdf_def.images[$(recn).data("short-code")] = "<?= http_method() . $_SERVER['SERVER_NAME'] . "/salons/$yearmonth/img/recognition/";?>" + $(recn).data("logo");
			});
			...
			pdfMake.createPdf(pdf_def).download("My_Results_<?= $contest['contest_name'];?>.pdf");
			hideLoader(this);

<?php
/*
** This program is used to generate a FB Share Image for the participant for International Salon 2020
*/
// PHP Settings
...
if (session_status() == PHP_SESSION_NONE) {
    ...
    if ( ! is_dir(__DIR__ . "/../../inc/session") )
        mkdir(__DIR__ . "/../../inc/session", 0755);
...
if ( ! empty($_SESSION['USER_ID']) && (! empty($_REQUEST['yearmonth'])) && (! empty($_REQUEST['profile_id'])) ) {
	...
	for ($idx = 0; $row = mysqli_fetch_array($query, MYSQLI_ASSOC); ++ $idx) {
		...
		// Get Image Sizes
		if ($contest_archived)
			list($img_width, $img_height, $img_type, $attr) = getimagesize("upload/$section/ar/$picfile");
		else
			list($img_width, $img_height, $img_type, $attr) = getimagesize("upload/$section/tn/$picfile");

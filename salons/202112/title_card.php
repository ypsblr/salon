...
class ImageMerge {
	...
	// const COLOR_SUBDUED = "CCCCCC";
	const COLOR_HIGHLIGHT = "F02020";
	const COLOR_GOLD = "A48438";
	const COLOR_WHITE = "FFFFFF";
	const COLOR_RED = "EE3537";
	const COLOR_MASK = "EFEFEF";
	const COLOR_THEME = "4285F4";		// matches blue in Golden Jubilee Logo
	// const COLOR_THEME = "FFFFFF";
	...
	const FILE_NAME_STUB = "IS2021-TITLE-CARD";
	const BASE_IMAGE = "is2021_titlecard.png";		// 1800w x 1200h - 6x3 inch card
	const DPI = 300;
	...
	protected $fields = [
							...
							// Profile Name
							array ("type" => "text", "field" => "profile_name", "value" => "", "function" => "format_name",
									"font" => self::FONT_BOLD, "font_size" => 64, "font_color" => self::COLOR_THEME,
									"x" => 400, "y" => 500, "width" => 800, "height" => 80, "rotate" => 0, "position" => "LT",
									"fill_color" => "", "border_size" => 0, "border_color" => ""),
							// Honors
							array ("type" => "text", "field" => "honors", "value" => "", "function" => "honors_text",
									"font" => self::FONT_ITALIC, "font_size" => 32, "font_color" => self::COLOR_SUBDUED,
									"x" => 400, "y" => 590, "width" => 800, "height" => 40, "rotate" => 0, "position" => "LT",
									"fill_color" => "", "border_size" => 0, "border_color" => ""),
							// Award Name
							array ("type" => "text", "field" => "award_name", "value" => "",
									"font" => self::FONT_BOLD, "font_size" => 48, "font_color" => self::COLOR_GOLD,
									"x" => 400, "y" => 660, "width" => 800, "height" => 60, "rotate" => 0, "position" => "LT",
									"fill_color" => "", "border_size" => 0, "border_color" => ""),
							// Picture Title
							array ("type" => "text", "field" => "title", "value" => "", "function" => "format_title",
									"font" => self::FONT_BOLD, "font_size" => 64, "font_color" => self::COLOR_THEME,
									"x" => 400, "y" => 750, "width" => 800, "height" => 80, "rotate" => 0, "position" => "LT",
							...
							array ("type" => "text", "field" => "section", "value" => "",
									...
									"x" => 400, "y" => 980, "width" => 800, "height" => 100, "rotate" => 0, "position" => "LB",
									"fill_color" => "", "border_size" => 0, "border_color" => ""),

							// flag
							// array ("type" => "vector", "field" => "flag", "value" => "",
							// 		"x" => 1280, "y" => 500, "width" => 300, "height" => 225, "rotate" => 0, "position" => "MM",
							// 		"fill_color" => "", "border_size" => 0, "border_color" => "", "border_gap" => 0),
							...
							array ("type" => "text", "field" => "country_name", "value" => "",
									"font" => self::FONT_BOLD, "font_size" => 64, "font_color" => self::COLOR_SUBDUED,
									"x" => 1280, "y" => 750, "width" => 300, "height" => 80, "rotate" => 0, "position" => "MT",
									"fill_color" => "", "border_size" => 0, "border_color" => ""),
	...
	function __construct($dbcon, $param) {
		...
		$this->sql .= "       honors, avatar, sortname, country_name, pic_result.pic_id, picfile, title, pic.section AS pic_section ";
		$this->sql .= "  FROM pic_result, profile, pic, country, ";
		$this->sql .= "       award LEFT JOIN section ON section.yearmonth = award.yearmonth AND section.section = award.section ";
		$this->sql .= " WHERE award.yearmonth = '" . self::YEARMONTH ."' ";
		$this->sql .= "   AND award.award_type = 'pic' ";
		$this->sql .= "   AND award.level < 99 ";
		// $this->sql .= "   AND award.section = 'CONTEST' ";					// Remove this test code
	...
	private function us_space($str) {
		return str_replace(" ", "_", $str);
	}

	// Invoke transformation function if defined
	private function transform($field, $value) {
		if (isset($this->fields[$field])) {
			if (isset($this->fields[$field]["function"])) {
				$function = $this->fields[$field]["function"];
	...
	function getMergeData() {
		...
		if ($row = mysqli_fetch_array($this->query)) {
			$this->target_folder = $_SERVER['DOCUMENT_ROOT'] . "/generated/" . self::YEARMONTH . "/titlecards/" . $row['section'];
			$this->save_file_name = sprintf("%s-%s-%s-%04d-%02d%s", self::FILE_NAME_STUB, $row['stub'], $this->us_space($row['award_name']), $row['profile_id'], $row['pic_id'], $this->save_file_type);
			foreach ($this->fields as $field => $config) {
				if ($config['field'] == 'picfile' && isset($row['section'])) {
					$picfile = $this->salon_folder . "/upload/" . $row['pic_section'] . "/" . $row['picfile'];
				...
				elseif ($config['field'] == "title")
					$this->fields[$field]["value"] = "'" . $this->transform($field, $row["title"]) . "'";
				elseif ($config['field'] == "section")
					$this->fields[$field]["value"] = $row["section"] . " Section";
				elseif ($config['field'] == "honors")
					$this->fields[$field]["value"] = $this->transform($field, $row["honors"]);
				elseif ($config['field'] == "flag") {
					// $flag_svg = $_SERVER['DOCUMENT_ROOT'] . "/plugin/flag-icon-css/flags/4x3/" . strtolower($row['sortname']) . ".svg";
					$flag_img = $_SERVER['DOCUMENT_ROOT'] . "/res/flag/" . strtolower($row['sortname']) . "_flag.png";
					if (file_exists($flag_img))
						$this->fields[$field]["value"] = $flag_img;
					else
						$this->fields[$field]["value"] = "";
				}
				else
					$this->fields[$field]["value"] = isset($row[$config['field']]) ? $this->transform($field, $row[$config['field']]) : "";
			}
			return $this->fields;
		}
		return NULL;
